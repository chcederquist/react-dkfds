...
export function Checkbox({ fieldsetProps, hint, id, legendProps, errorMessage, values, name }: Readonly<CheckboxProps>) {
  return (
    ...
    <fieldset aria-describedby={mergeStrings(hint && id+'-hint', errorMessage && id+'-error')} {...fieldsetProps}>
        ...
        {values && values.map((value) => (
          <div className="form-group-radio" key={value.id}>
            <input type="checkbox" id={value.id} name={name}
                className="form-checkbox" value={value.value} />
